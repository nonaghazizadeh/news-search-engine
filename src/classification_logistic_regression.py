...
class ClassificationLogisticRegression:
    ...
    def load_fasttext_embedding(self):
        with open(Path.CLASSIFICATION_LR_FASTTEXT_EMBEDDING_PATH.value, 'r',
                  encoding="utf-8") as f:
            self.fasttext_docs_embedding = json.loads(f.read())

    def load_fasttext(self):
        self.fasttext_model = fasttext.load_model(Path.CLASSIFICATION_LR_FASTTEXT_MODEL_PATH.value)
    ...
    def save_naive_bayes(self):
        with open(Path.CLASSIFICATION_LR_PATH.value, 'wb') as f:
            pickle.dump(self.model, f)

    def load_naive_bayes(self):
        with open(Path.CLASSIFICATION_LR_PATH.value, 'rb') as f:
    ...
    def visualize_naive_bayes_csv(self, y_predicted):
        ...
        with open(Path.CLASSIFICATION_LR_RESULT_PATH.value, 'w', newline='') as file:
